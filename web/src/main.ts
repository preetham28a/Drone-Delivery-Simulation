...
const saveSimulationButton = $("#save-simulation")[0];
const restoreSimulationButton = $("#restore-simulation")[0];
const resetSimulationButton = $("#reset-simulation")[0];
...
saveSimulationButton.onclick = () => {
  sendCommand("saveSimulation", {});
};

restoreSimulationButton.onclick = () => {
  sendCommand("restoreSimulation", {});
};
resetSimulationButton.onclick = () => {
  sendCommand("resetSimulation", {});
  loadScene(sceneFile);
};
  ...
  socket.onmessage = (msg) => {
    ...
    switch (data.event) {
      ...
      case "SimulationSaved":
        notify("Simulation state saved successfully.");
        break;
      case "SimulationRestored":
        notify("Simulation state restored successfully.");
        break;
    }
